export function generateBoard(values, main) {
    ...
    for(let i = 0; i < 9; i++) {
        ...
        for(let j = 0; j < 9; j++ ) {
            ...
            const rowIndeks= rowOffset + Math.floor(j / 3) ;

            rows[rowIndeks][colIndex] = cell ;
            columns[colIndex][rowIndeks] = cell ;
        }
    }
...
function block(values) {    
    ...
    return {
        cells: [ ...row1.cells, ...row2.cells, ...row3.cells ], // Тук от трите масива с по три клетки, получаваме един масив с девет клетки
        element
    }
}
...
// Това е функцията за съьдаване на елементи от HTML в DOM дървото.
