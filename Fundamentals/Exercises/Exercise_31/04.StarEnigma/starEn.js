function starEnigma(inp) {
    ...
    //let regex = new RegExp('.*?@([A-Za-z]+).*?:(\\d+).*?!(A|D)!.*?->(\\d+)');
    let regex = new RegExp('(?<g1>.*?)@(?<planet>[A-Za-z]+)(?<g2>.*?):(\\d+)(?<g3>.*?)!(?<attack>A|D)!(?<g4>.*?)->(\\d+)(?<real>[\\.\\d+]*)');
    ...
    for ( let i = 0 ; i < messNum ; i ++ ) {
        ...
        //     count = row.length ;
        // }
            let count = row&&row.length ;
            //let count = (row && row.length) || 0 ;


            r = r.split('').map( (a) => String.fromCharCode( a.charCodeAt(0) - count)).join('');
            let line = regex.exec(r,'g') ;
            if (line !== null && line.groups.real === '') {
                let spr = (line.groups.g1 + line.groups.g2 + line.groups.g3 + line.groups.g4).split('') ;
